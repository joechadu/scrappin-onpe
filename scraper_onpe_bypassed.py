...
def scrape_onpe_santa_anita_bypassed():
    ubigeo = "140143"
    ...
    print(f"Iniciando scraping (con curl_cffi bypass) de actas para el Ubigeo {ubigeo}...")
    ...
    while True:
        ...
        try:
            res_actas = requests.get(url_actas, headers=headers, impersonate="chrome110")
            
            if "html" in res_actas.headers.get("Content-Type", ""):
                print(f"ALERTA: La API devolvió HTML en la página {pagina}.")
                break
            ...
            # The API returns a direct string sometimes if error, we need to ensure it's dict/list
            ...
            # If actas is somehow a string still, break
            ...
            for acta_resumen in actas:
                ...
                if 'detalle' not in acta_resumen:
                    ...
                    res_detalle = requests.get(url_detalle, headers=headers, impersonate="chrome110")
...
if __name__ == "__main__":
    scrape_onpe_santa_anita_bypassed()
